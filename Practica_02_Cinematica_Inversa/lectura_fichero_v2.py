...
def leer_archivo(input_file):
    ...
    for i in range(articulations):
        reader = input_file.readline()  # Tipo de articulación
        if reader[-1] == "\n":
            reader = reader[:-1]
        ...
        for parameter in ["Angulo", "Longitud", "Límite"]:
            reader = input_file.readline()
            if reader[-1] == "\n":
                reader = reader[:-1]
            if not reader.isdigit() or int(reader) < 0:
                print(
                    f"{parameter} de las articulaciones deben ser numéricos y mayores que cero")
                exit(1)
            if parameter == "Angulo":
                th.append(int(reader))
            elif parameter == "Longitud":
                a.append(int(reader))
            elif parameter == "Límite":
                limits.append(int(reader))

    return articulations, th, a, type, limits
...
with open(sys.argv[3]) as input_file:
    articulations, th, a, type, limits = leer_archivo(input_file)
...
O = [None] * (len(th) + 1)  # Utilizamos una lista en lugar de range
O[0] = cin_dir(th, a)  # Calculamos la posición inicial
...
while dist > EPSILON and abs(prev-dist) > EPSILON/100.:
    ...
    for i in range(len(th)):
        ...
        if type[-1 - i] == "rotation":
            ...
            # Cálculo de 'd'
            d = np.dot([cos(w), sin(w)], [objetivo[0] - O[i]
                       [-1][0], objetivo[1] - O[i][-1][1]])
